...
if __name__ == "__main__":
    ...
    print("随机重启爬山法")
    success_num = int(0)
    sum_expand_times_success = int(0)
    sum_expand_times_failure = int(0)
    time_used = []
    num_expanded = []
    for t in range(T):
        board = NQueensProblem(N=n, state=tuple(random.choice(range(n)) for _ in range(n)))
        T1 = time.perf_counter()
        res = hill_climbing_random_restart(board)
        T2 = time.perf_counter()
        time_used.append((T2-T1)*1000)
        if res["solved"]:
            sum_expand_times_success += res["expanded"]
            success_num += 1
        else:
            sum_expand_times_failure += res["expanded"]
        num_expanded.append(res["expanded"])
    time_used = np.array(time_used, np.double)
    num_expanded = np.array(num_expanded, np.int32)
    print("平均探索的节点数±标准差: " + str(np.mean(num_expanded)) + "±" + str(np.std(num_expanded)) + "次")
    print("平均运行时间±标准差: " + str(np.mean(time_used)) + "±" + str(np.std(time_used)) + "ms")
    print("成功率 = " + str(success_num / T))
    print("失败率 = " + str(1 - success_num / T))
    print("成功时的平均探索的节点数 = " + str(sum_expand_times_success / success_num))
    print("失败时的平均探索的节点数 = " + str(sum_expand_times_failure / (T-success_num)))